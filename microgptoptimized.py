...
for step in range(3000):
    ...
    if loss.data < 1.7:
        print(f"Early stopping at step {step+1} as loss ({loss.data:.4f}) is below 1.7")
        break
    ...
    for i, p in enumerate(params):
        ...
        p.grad = 0
    if step % 200 == 0: print(f"step {step+1} | loss {loss.data:.4f}")

# --- Generation / Sampling (Temp 0.7) ---
temperature = 0.7
for _ in range(10):
    out = []
    keys, values = [[] for _ in range(n_layer)], [[] for _ in range(n_layer)]
    ...
    for pos_id in range(block_size):
        logits = gpt(curr_token, pos_id, keys, values)
        t_logits = [l / temperature for l in logits]
        probs = softmax(t_logits)
        p_data = [p.data for p in probs]
        curr_token = random.choices(range(vocab_size), weights=p_data)[0]
        if curr_token == BOS: break
        out.append(uchars[curr_token])
    print(''.join(out))
