"""
MASC 515 - Enhanced microgpt (Atomic Version)
Features: GELU, LoRA, RoPE, and MoE implemented in pure Python.
"""
...
# Note: 'wpe' (positional embeddings) removed in favor of RoPE
...
for i in range(n_layer):
    ...
    # [FEATURE 2: LoRA Parameters]
    # Applied to the Query projection for efficiency
    state_dict[f'layer{i}.attn_wq_lora_a'] = matrix(lora_r, n_embd)
    state_dict[f'layer{i}.attn_wq_lora_b'] = matrix(n_embd, lora_r)
    
    # [FEATURE 3: MoE Parameters]
    # Two experts for the MLP block
    state_dict[f'layer{i}.moe_gate'] = matrix(2, n_embd) # Gating for 2 experts
    for e in range(2):
...
def gpt(token_id, pos_id, keys, values):
    x = rmsnorm(state_dict['wte'][token_id]) # RoPE doesn't use wpe addition
    
    for li in range(n_layer):
        x_res = x
        x = rmsnorm(x)
        
        # 1) Attention with LoRA (on Q)
        ...
        for h in range(n_head):
            ...
            attn_weights = softmax([sum(q_h[j] * k_h[t][j] for j in range(head_dim)) / head_dim**0.5 for t in range(len(k_h))])
        ...
        # 2) [FEATURE 3: Mixture of Experts MLP]
        x_res = x
        x = rmsnorm(x)
        ...
        for e in range(2):
            ...
            expert_x = [xi.gelu() for xi in expert_x] # [GELU used here]
...
for step in range(100): # Shortened for verification
